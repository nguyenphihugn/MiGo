...
const firstpage = async () => {
  const res = await fetch(
    `https://api.themoviedb.org/3/discover/movie?api_key=${myApi}&sort_by=vote_count.desc&vote_average.gte=8&vote_average.lte=10&include_adult=false&page=${intialPage}&with_genres=${categoryId}&language=vi-VN`
  );
  const data = await res.json();
